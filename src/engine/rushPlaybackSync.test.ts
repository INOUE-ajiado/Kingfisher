...
describe('オペレーターの操作 → 全員に配る再生状態', () => {
  const playing = { playing: true, position: 3 };
  const paused = { playing: false, position: 3 };

  it('再生中に押すと止まり、止まっているときに押すと再生する', () => {
    expect(commandToggle(playing, 5, 10)).toEqual({ playing: false, position: 5 });
    expect(commandToggle(paused, 5, 10)).toEqual({ playing: true, position: 5 });
  });
  ...
  it('シークは再生中かどうかを変えない', () => {
    expect(commandSeek(playing, 7, 10)).toEqual({ playing: true, position: 7 });
    expect(commandSeek(paused, 7, 10)).toEqual({ playing: false, position: 7 });
  });

  it('コマ送りは止めてから動かす', () => {
    expect(commandStep(playing, 3, 1, 24, 10)).toEqual({ playing: false, position: 3 + 1 / 24 });
...
describe('書き込みの間引き', () => {
  const s = (position: number) => ({ playing: true, position });
