 ...
 * 色指定・作画比較用の参照画像ウィンドウ。
 *
 * セルウィンドウ内にドッキングした状態と、画面上を自由に動かせる
 * フローティング状態の両方を同じコンポーネントで扱う。
 * ヘッダーのドラッグ量で「引きはがし (tear-off)」と「再ドッキング」を判定する。
 */
export const ReferenceCanvasView: React.FC<{ isFloating?: boolean }> = React.memo(({ isFloating = false }) => {
